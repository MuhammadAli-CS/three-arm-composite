...
% ------------------------------------------------------------------
\begin{frame}[shrink=6]{What the two factors actually compute}
...
% ------------------------------------------------------------------
\begin{frame}[shrink=8]{All 19 conditions}
...
\toprule
Map & $g$ & What $g$ destroys & & $\Ulost$ & obs.\ $h$ & know $g$ \\
\midrule
\multicolumn{7}{l}{\textbf{Controls} --- answers known before the run}\\
\texttt{identity}   & $z$          & nothing                      &                 & $0.000$ & \good{$0.000^\ast$} & $+0.41$ \\
\texttt{exp\_b1}    & $e^{z}$      & nothing (monotone)           &                 & $0.000$ & \good{$0.000^\ast$} & $+0.63$ \\
\texttt{sinh\_b1}   & $\sinh z$    & nothing (monotone)           &                 & $0.000$ & \good{$0.000^\ast$} & $+0.44$ \\
\texttt{sum}        & $y_1+y_2$    & how the sum splits           & \emph{linear}   & $0.702$ & $-0.18$ & \good{$+0.12$} \\
\midrule
\multicolumn{7}{l}{\textbf{The sweep} --- one form, loss tuned by $c$; $\Ulost$ \emph{at iteration 0} in brackets}\\
\texttt{shiftsq\_c0}   & $z^2$       & sign of $z$     & $[1.03]$ & $0.280$ & \bad{$+1.49$} & $+0.12$ \\
\texttt{shiftsq\_c0.5} & $(z-0.5)^2$ & sign of $z-0.5$ &          & $0.283$ & \bad{$+1.28$} & $+0.63$ \\
\texttt{shiftsq\_c1}   & $(z-1)^2$   & sign of $z-1$   & $[0.87]$ & $0.283$ & \bad{$+1.89$} & $-0.10$ \\
\texttt{shiftsq\_c2}   & $(z-2)^2$   & rarely ambiguous& $[0.10]$ & $0.189$ & \bad{$+2.75$} & $-0.73$ \\
\texttt{shiftsq\_c3}   & $(z-3)^2$   & almost nothing  & $[0.01]$ & $0.232$ & \bad{$+2.51$} & $-1.18$ \\
\midrule
\multicolumn{7}{l}{\textbf{Matched pairs} --- one property changed at a time}\\
\texttt{abs}      & $|z|$                     & sign of $z$, with a kink  &                    & $0.474$ & $-0.06$ & $-0.01$ \\
\texttt{cos\_w1}  & $\cos z$                  & which of few preimages    &                    & $2.434$ & $+2.45$ & $-0.79$ \\
\texttt{cos\_w2}  & $\cos 2z$                 & which of more preimages   &                    & $1.284$ & $+1.06$ & $+0.70$ \\
\texttt{cos\_w4}  & $\cos 4z$                 & which of many preimages   &                    & $1.161$ & $+1.08$ & $-0.80$ \\
\texttt{relu}     & $-\max(z,0)$              & all $z<0$, \emph{away from} opt. &             & $0.075$ & \good{$-0.07$} & $+0.20$ \\
\texttt{clip\_c2} & $\text{clip}(2y_1)+y_2$   & clipped coord., \emph{at} opt.   &             & $0.580$ & \good{$+1.11$} & $+0.28$ \\
\midrule
\multicolumn{7}{l}{\textbf{Benchmark analogs} --- \texttt{sum} above is the Truss / RCM40 analog}\\
\texttt{product} & $y_1y_2$               & factoring, and sign   & DTLZ2        & $1.640$ & $+0.21$ & $-0.41$ \\
\texttt{norm}    & $\sqrt{y_1^2+y_2^2}$   & direction             & Color        & $0.426$ & $-0.10$ & $-0.02$ \\
\texttt{max}     & $\max(y_1,y_2)$        & the smaller value     & Lake         & $0.358$ & $+0.14$ & $+0.07$ \\
\texttt{ratio}   & $u_1/(0.05+u_1+u_2)$   & scale                 & RGB          & $0.267$ & $+0.12$ & $-0.42$ \\
\bottomrule
\end{tabular}
\end{center}

\vspace{2pt}
{\tiny Differences in final $\log_{10}$ regret, $>0$ favours the arm that sees
more. ``obs.\ $h$'' has the measured $+0.41$ bias subtracted;
$^\ast$ exact, no correction needed. SEs $0.06$--$0.48$.}
...
% ------------------------------------------------------------------
\begin{frame}[shrink=12]{The finding: \emph{where}, not \emph{how much}}

Both maps censor, and \texttt{relu} discards strictly more of the domain
--- an entire half-line collapses to one value. But \emph{where} it discards it
is different:
...
\bigskip
\begin{block}{$\Ulost$ already encodes this}
It is measured at the points BO \emph{actually evaluated}, so a fiber that sits away from the optimum barely registers --- $0.075$ against $0.580$.
\end{block}
...
% ------------------------------------------------------------------
\begin{frame}[shrink=5]{Diagnosis so far}
...
% ------------------------------------------------------------------
\begin{frame}{Both effects, every condition}
...
\vspace{-4pt}
{\footnotesize Error bars are paired standard errors over the 20 (draw, seed)
pairs. Note the sweep: \textcolor{blue!45!black}{knowing $g$} turns
\emph{negative} as $c$ grows, which cannot happen if arm 3 is working --- it
holds the true kernel \emph{and} knows $g$.}

\end{frame}

% ------------------------------------------------------------------
\begin{frame}{The plot this study was built to produce}

\begin{columns}[c]
\begin{column}{0.62\textwidth}
\includegraphics[width=\linewidth]{../figures/hypothesis.pdf}
\end{column}
\begin{column}{0.36\textwidth}
\small
The hypothesis predicts an upward trend.

\medskip
Over all 16 lossy maps there is none.

\medskip
Dropping the sweep produces $r=+0.78$ --- but dropping the $\cos$ family as
well leaves $+0.15$, so the apparent trend rests on three points.

\medskip
\textbf{No line is fitted}, deliberately: a regression here would assert a
relationship the data do not support.
\end{column}
\end{columns}
...
% ------------------------------------------------------------------
\begin{frame}{The sweep, and why it fails}
...
\vspace{-2pt}
{\footnotesize Right panel is the diagnosis: at $c=3$ the latent arm starts
knowing $h$ almost exactly ($\Ulost = 0.004$) and ends two orders of magnitude
worse. Small $c$ behaves correctly --- uncertainty falls as data arrives. More
data cannot increase uncertainty about points already observed.}

\end{frame}

% ------------------------------------------------------------------
\begin{frame}{The finding, in one figure}

\begin{center}
\includegraphics[width=0.88\linewidth]{../figures/censoring.pdf}
\end{center}

\vspace{-4pt}
{\footnotesize $-\max(z,0)$ has the \emph{larger} fiber --- an entire half-line
--- but it sits away from the optimum, so little is lost where BO looks and
observing $h$ buys nothing. The clipped pair censors where the optimizer is
working, and observing $h$ is worth $13\times$ in final regret.}

\end{frame}

% ------------------------------------------------------------------
\begin{frame}{Convergence, three arms}
...
\vspace{-2pt}
{\footnotesize Median over the 20 pooled runs, and \textbf{uncorrected} --- so
each latent--composite gap here still contains the $+0.41$ handicap that the
\texttt{identity} control measures. On \texttt{relu} that handicap is most of
the visible gap; on \texttt{clip\_c2} and \texttt{shiftsq\_c0} it is a small
part of it.}

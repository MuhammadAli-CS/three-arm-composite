...
The main grid is pending at the time of writing; it is produced by
\texttt{run.py --collect}, which pools the function draws and recomputes the
paired statistics over the pooled (draw, seed) pairs. The quantities to read off
it are: ``observing $h$'' against $\Ulost$ along the sweep; ``knowing $g$'' on
the linear map (predicted $\approx0$); ``observing $h$'' on the injective maps
(predicted exactly $0$); the rectified-versus-clipped contrast; and the analogs
against the corresponding benchmark results.
